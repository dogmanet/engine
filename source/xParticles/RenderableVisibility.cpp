...
CRenderableVisibility::CRenderableVisibility(ID idPlugin, CParticleSystem *pSystem):
	m_idPlugin(idPlugin),
	m_pSystem(pSystem)
{
}
...
void CRenderableVisibility::updateForCamera(IXCamera *pCamera, const IXRenderableVisibility *pReference)
{
	...
	m_pSystem->computeVisibility(pFrustum, pCamera->getLook(), this, pCamera->getLayerMask(), pRef, pCamera);
...
void CRenderableVisibility::updateForFrustum(const IXFrustum *pFrustum, UINT bmLayers, const IXRenderableVisibility *pReference)
{
	...
	m_pSystem->computeVisibility(pFrustum, float3(), this, bmLayers, pRef);
}

static void SortRenderList(Array<CParticlePlayerEmitter*> &aList)
{
	/*XPROFILE_FUNCTION();
...
static void MergeArrays(Array<CParticlePlayerEmitter*> &aTo, const Array<CParticlePlayerEmitter*> &aFrom)
{
	if(!aFrom.size())
	{
		return;
...
static void SetRenderList(Array<CParticlePlayerEmitter*> &aList, void **ppData, UINT uCount, bool useSort=true)
{
...
Array<CParticlePlayerEmitter*>& CRenderableVisibility::getRenderList()
{
	return(m_aRenderList);
}

Array<CParticlePlayerEmitter*>& CRenderableVisibility::getSelfillumList()
{
	return(m_aSelfillumList);
}

/*Array<CParticlePlayerEmitter*>& CRenderableVisibility::getTransparentList()
{
