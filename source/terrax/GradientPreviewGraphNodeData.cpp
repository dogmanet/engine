...
void CGradientPreviewGraphNodeData::update(IXColorGradient *pGradient)
{
	m_pRenderer->reset();
...
void CGradientPreviewGraphNodeData::drawArea(IXColorGradient *pGradient)
{
	m_pRenderer->setColor(float4(0.0f, 1.0f, 0.0f, 0.5f));

	UINT uMin = 0;
	UINT uMax = m_uWidth;
	float2_t vAreaSize((float)m_uWidth, (float)m_uHeight);
	...
	for(UINT x = uMin; x <= uMax; x += 1)
	{
		...
		if(x != uMin)
		{
			float3_t vPt0((float)(x - 1), 0.0f, 0.0f);
			float3_t vPt1((float)(x - 1), vAreaSize.y + 1.0f, 0.0f);
			float3_t vPt2((float)x, vAreaSize.y + 1.0f, 0.0f);
			float3_t vPt3((float)x, 0.0f, 0.0f);
